...
def calculate():
    ...
    try:
        ...
        entry.delete(0, tk.END)
        entry.insert(tk.END, str(round(result, 3)))
...
def apply_function(func):
    try:
        ...
        if func == "sin":
            ...
            result = 1 / val if val != 0 else "Error"
        else:
            result = "Unknown"

        entry.delete(0, tk.END)
        entry.insert(tk.END, str(round(result, 5)))
